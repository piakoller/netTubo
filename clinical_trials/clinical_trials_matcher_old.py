...
# Add the parent directory to Python path to import shared_logic
...
class PatientStudyMatcher:
    ...
    def __init__(self, llm_matcher: LLMStudyMatcher, studies_files: List[str] = None):
        self.llm_matcher = llm_matcher
        self.studies_files = studies_files or []
    ...
    def load_combined_studies(self, studies_files: List[str]):
        ...
        for studies_file in studies_files:
            try:
                logger.info(f"Loading studies from: {studies_file}")
                with open(studies_file, 'r', encoding='utf-8') as f:
                    data = json.load(f)
                
                studies_data = data.get('studies', [])
                
                # Determine dataset source from filename
                if 'HasResults-false' in studies_file:
                    dataset_source = 'HasResults-false'
                    has_posted_results = False
                elif 'HasResults-true' in studies_file:
                    dataset_source = 'HasResults-true'
                    has_posted_results = True
                else:
                    dataset_source = 'Unknown'
                    has_posted_results = True
                
                # Convert study data to expected format
                for study_data in studies_data:
                    # Add dataset source information
                    study_data['dataset_source'] = dataset_source
                    study_data['has_posted_results_original'] = has_posted_results
                    ...
                    study = {
                        ...
                        "hasResults": study_data.get('has_posted_results', has_posted_results),
                        "dataset_source": dataset_source,
                        "original_study_data": study_data  # Keep original data for reference
                    }
                    combined_studies.append(study)
                
                logger.info(f"Loaded {len(studies_data)} studies from {studies_file}")
                
            except Exception as e:
                logger.error(f"Error loading studies from {studies_file}: {e}")
                continue
        
        # Remove duplicates based on NCT ID, preferring HasResults-true version
        unique_studies = {}
        for study in combined_studies:
            ...
            if nct_id:
                ...
                elif study.get('dataset_source') == 'HasResults-true':
                    # Prefer HasResults-true version if we have a duplicate
                    unique_studies[nct_id] = study
        
        self.pre_filtered_studies = list(unique_studies.values())
        
        # Log dataset composition
        hasresults_false_count = len([s for s in self.pre_filtered_studies if s.get('dataset_source') == 'HasResults-false'])
        hasresults_true_count = len([s for s in self.pre_filtered_studies if s.get('dataset_source') == 'HasResults-true'])
        
        logger.info(f"Combined dataset loaded: {len(self.pre_filtered_studies)} unique studies")
    ...
    def find_relevant_studies(self, 
        ...
        logger.info(f"Using {len(self.pre_filtered_studies)} combined pre-filtered studies from multiple datasets")
...
def main():
    ...
    # Use combined pre-filtered studies instead of single API search
    matcher = PatientStudyMatcher(llm_matcher=llm_matcher, studies_files=STUDIES_FILES)
    ...
    summary_report = [
        ...
        "="*80,
        "Note: Used combined pre-filtered studies from HasResults-false and HasResults-true datasets",
        "with publications from 2020 onwards and posted results. LLM evaluation assessed clinical",
        "relevance based on patient diagnosis and clinical context. No real-time API calls were made."
    ]
